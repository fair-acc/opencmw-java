...
class OpenCmwCppInteropTest {
    ...
    void setupBrokerAndEventStore() throws IOException {
        OpenCmwDataSource.class.getClass(); // just make sure the static registration inside the class is run
    ...
    void testSerialiserCompatibility() throws Exception {
        ...
        cppOpencmw.waitFor();
        String opencmwCppString = new String(cppOpencmw.getInputStream().readAllBytes());
        Files.write(new File("cpp.hex").toPath(), opencmwCppString.getBytes());
        ...
        final FastByteBuffer cppSerialisedBuffer = FastByteBuffer.wrap(opencmwCppString.getBytes());
    ...
    public static class TestContext implements Filter {
        ...
        public void copyTo(Filter other) {
            ...
            // otherContext.contextA = contextA;
            // otherContext.contextB = contextB;
