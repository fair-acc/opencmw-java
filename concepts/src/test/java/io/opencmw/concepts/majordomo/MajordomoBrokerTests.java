...
public class MajordomoBrokerTests {
    ...
    public void basicLowLevelRequestReplyTest() throws InterruptedException, IOException {
        ...
        // test interfaces
        Assertions.assertNotNull(broker.getContext());
        Assertions.assertNotNull(broker.getInternalRouterSocket());
        Assertions.assertNotNull(broker.getServices());
        assertEquals(2, broker.getServices().size());
        Assertions.assertDoesNotThrow(() -> broker.addInternalService(new MajordomoWorker(broker.getContext(), "demoService"), 10));
        assertEquals(3, broker.getServices().size());
        Assertions.assertDoesNotThrow(() -> broker.removeService("demoService"));
        assertEquals(2, broker.getServices().size());
        ...
        Assertions.assertNotNull(reply.toString());
        assertNotNull(reply, "reply message w/o RBAC token not being null");
        Assertions.assertTrue(reply instanceof MdpClientMessage);
        MdpClientMessage clientMessage = (MdpClientMessage) reply;
        Assertions.assertNull(clientMessage.senderID); // default dealer socket does not export sender ID (only ROUTER and/or enabled sockets)
        ...
        Assertions.assertNull(clientMessage.getRbacFrame());
    ...
    public void basicSynchronousRequestReplyTest() throws InterruptedException, IOException {
        ...
        Assertions.assertDoesNotThrow(() -> clientSession.setRetries(4));
        assertEquals(4, clientSession.getRetries());
        assertEquals(2500, clientSession.getTimeout());
        Assertions.assertDoesNotThrow(() -> clientSession.setTimeout(2000));
        assertEquals(2000, clientSession.getTimeout());
        Assertions.assertNotNull(clientSession.getUniqueID());

        {
            ...
            Assertions.assertNotNull(replyWithoutRbac, "reply message w/o RBAC token not being null");
            Assertions.assertNotNull(replyWithoutRbac.peekLast(), "user-data not being null");
            Assertions.assertArrayEquals(DEFAULT_REQUEST_MESSAGE_BYTES, replyWithoutRbac.pollLast().getData(), "equal data");
        }
...

        {
            ...
            Assertions.assertNotNull(replyWithRbac, "reply message with RBAC token not being null");
            Assertions.assertNotNull(replyWithRbac.peekLast(), "RBAC token not being null");
            Assertions.assertArrayEquals(DEFAULT_RBAC_TOKEN, replyWithRbac.pollLast().getData(), "equal RBAC token");
            Assertions.assertNotNull(replyWithRbac.peekLast(), "user-data not being null");
            Assertions.assertArrayEquals(DEFAULT_REQUEST_MESSAGE_BYTES, replyWithRbac.pollLast().getData(), "equal data");
        }
    ...
    public void basicMmiTests() throws IOException {
...

        {
            ...
            Assertions.assertNotNull(replyWithoutRbac, "reply message w/o RBAC token not being null");
            Assertions.assertNotNull(replyWithoutRbac.peekLast(), "user-data not being null");
            Assertions.assertArrayEquals(DEFAULT_REQUEST_MESSAGE_BYTES, replyWithoutRbac.pollLast().getData(), "MMI echo service request");
        }

        {
            ...
            Assertions.assertNotNull(replyWithoutRbac, "reply message w/o RBAC token not being null");
            Assertions.assertNotNull(replyWithoutRbac.peekLast(), "user-data not being null");
            Assertions.assertEquals("200", new String(replyWithoutRbac.pollLast().getData(), StandardCharsets.UTF_8), "known MMI service request");
        }
...

        {
            ...
            Assertions.assertNotNull(replyWithoutRbac, "reply message w/o RBAC token not being null");
            Assertions.assertNotNull(replyWithoutRbac.peekLast(), "user-data not being null");
            Assertions.assertEquals("400", new String(replyWithoutRbac.pollLast().getData(), StandardCharsets.UTF_8), "unknown MMI service request");
        }

        {
            ...
            Assertions.assertNotNull(replyWithoutRbac, "reply message w/o RBAC token not being null");
            Assertions.assertNotNull(replyWithoutRbac.peekLast(), "user-data not being null");
            Assertions.assertEquals("501", new String(replyWithoutRbac.pollLast().getData()), "unknown service");
        }
    ...
    public void basicASynchronousRequestReplyTest() throws IOException {
        ...
        new Thread(() -> {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                Assertions.assertNotNull(reply, "reply message w/o RBAC token not being null");
                Assertions.assertNotNull(reply.peekLast(), "user-data not being null");
                Assertions.assertArrayEquals(DEFAULT_REQUEST_MESSAGE_BYTES, reply.getLast().getData());
                counter.getAndIncrement();
            }
        }).start();

        Awaitility.await().alias("wait for reply messages").atMost(1, TimeUnit.SECONDS).until(counter::get, Matchers.equalTo(10));
        Assertions.assertEquals(10, counter.get(), "received expected number of replies");
